...
3. **FAQ handling** — business profile (hours, services, prices, common questions) that the agent draws answers from.
4. **Booking** — agent checks availability and books a real appointment slot for one seeded service.
5. **Human handoff** — when the AI can't resolve something: log the request, notify staff via Telegram with a transcript summary, tell the caller "someone will follow up shortly."
6. **STT/TTS/LLM** - build the pipeline
7. **Local language support** — Amharic (and Afan Oromo if time allows) via Addis AI's.
8. **Embeddable widget** — the same call widget as an iframe snippet, so it can be dropped onto any business's landing page.
...
3. Judge asks an FAQ ("what time do you close?") — AI answers from the business profile.
4. Judge books an appointment — AI checks availability and confirms a slot.
5. Judge asks something out of scope — AI logs it, tells the caller someone will follow up, and a Telegram message lands on the "staff" phone showing the handoff in real time.
